...
from app.models.publication import ContentType, Event, Publication, SocialScale, StaffProfile, Story, Submission, EventAttachment, StaffReaction, Comment, CommentReaction, CommentMention, Contest, JudgeAssignment, JudgeScore
...
async def list_contests(
    ...
    if search:
        ...
        query = query.filter(Contest.title.ilike(f"%{escaped}%"))
...
@router.post("/contests/{contest_id}/submissions/bulk-status")
async def bulk_update_submission_status(
    contest_id: int,
    payload: SubmissionBulkStatusUpdate,
    db: Session = Depends(get_db),
    ...
    for sub in submissions:
        ...
        try:
            ...
            user = db.query(UserModel).filter(UserModel.email == sub.student_email).first()
            if user:
                if payload.status == 'approved':
                    n = Notification(
                        user_id=user.id,
                        title=f"Bài dự thi được duyệt",
                        body=f"Bài \"{sub.title}\" của bạn đã được duyệt trong cuộc thi \"{contest.title}\".",
                        url=f"/cuoc-thi/{contest.slug}?tab=bai-thi",
                    )
                elif payload.status == 'rejected':
                    reason = payload.rejection_reason or "Không đạt yêu cầu"
                    n = Notification(
                        user_id=user.id,
                        title=f"Bài dự thi bị từ chối",
                        body=f"Bài \"{sub.title}\" đã bị từ chối. Lý do: {reason}",
                        url=f"/cuoc-thi/{contest.slug}?tab=sang-tac",
                    )
                else:
                    n = None
                if n:
    ...
    db.commit()
    return {"message": f"Updated {updated} submissions", "updated": updated}


@router.post("/contests/{contest_id}/submissions/bulk-delete")
async def bulk_delete_submissions(
    contest_id: int,
    payload: SubmissionBulkDelete,
    db: Session = Depends(get_db),
    ...
    for sub in submissions:
        if sub.attachment_url:
            try:
                old_filename = sub.attachment_url.split("/")[-1]
        ...
        db.query(SubmissionVote).filter(SubmissionVote.submission_id == sub.id).delete()
        db.delete(sub)
        deleted += 1

    contest.submission_count = max(0, (contest.submission_count or 0) - deleted)
    db.commit()
    return {"message": f"Deleted {deleted} submissions", "deleted": deleted}


@router.post("/contests/{contest_id}/judges")
async def assign_judge(
    ...
    existing = db.query(JudgeAssignment).filter(
        JudgeAssignment.contest_id == contest_id,
    ...
    assignment = JudgeAssignment(contest_id=contest_id, user_id=payload.user_id)
    db.add(assignment)

    try:
        n = Notification(
            user_id=payload.user_id,
            title=f"Bạn được phân công giám khảo",
            body=f"Bạn đã được phân công làm giám khảo cho cuộc thi \"{contest.title}\".",
            url=f"/cuoc-thi/{contest.slug}?tab=giam-khao",
        )
        db.add(n)
    except Exception:
        pass
    ...
    return {"message": "Judge assigned successfully", "id": assignment.id}


@router.delete("/contests/{contest_id}/judges/{user_id}")
...
@router.get("/contests/{contest_id}/judges")
async def list_judges(
    ...
    assignments = db.query(JudgeAssignment).filter(JudgeAssignment.contest_id == contest_id).all()
    result = []
    for a in assignments:
        ...
        result.append({
            "id": a.id,
            "user_id": a.user_id,
            "judge_name": user.full_name if user else None,
            "judge_email": user.email if user else None,
...
async def get_contest_results(
        ...
        if has_judging:
            ...
            for crit_idx, crit in enumerate(judging_criteria):
                ...
                if crit_scores:
                    ...
                    entry["criteria_scores"][crit.get('name', f'Criterion {crit_idx}')] = {
                        "average": round(avg, 2),
                        "count": len(crit_scores),
                        "weight": weight,
                    }
    ...
    return {
        ...
        "total_judges": db.query(JudgeAssignment).filter(JudgeAssignment.contest_id == contest_id).count(),
    }
