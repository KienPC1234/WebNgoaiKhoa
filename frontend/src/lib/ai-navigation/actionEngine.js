...
const hasPostLookupSignals = (query = '') => {
  const normalizedQuery = normalize(query)
  if (!normalizedQuery) return false
  return /\bbai viet\b|\bpost\b|\ban pham\b|\barticle\b/.test(normalizedQuery)
}
...
const selectBestContentMatch = ({ matches = [], query = '', currentPath = '' }) => {
  ...
  const hasPostIntent = hasPostLookupSignals(queryText)
  ...
  for (const item of matches) {
    ...
    if (hasPostIntent && (item?.entity_type === 'publication' || path.startsWith('/posts/'))) score += 34
    ...
    if (hasPostIntent && pathname.startsWith('/doingu/')) score -= 70
...
const resolveSearchIntent = async (query, limit) => {
  ...
  const titleIntent = hasTitleLookupSignals(trimmedQuery)
  const postIntent = hasPostLookupSignals(trimmedQuery)
  ...
  if (knowledgeMatches.length > 0) {
    const preferredKnowledge = (titleIntent || postIntent)
      ? (knowledgeMatches.find((item) => item?.source_type === 'publication') || null)
      : null
    const selectedKnowledge = preferredKnowledge || knowledgeMatches[0]
    const mapped = mapKnowledgeResultToRoute(selectedKnowledge)
    if (mapped) {
      return {
        ...mapped,
        target: mapped.target || (focusPhrase ? `text:${focusPhrase}` : null),
        targetLabel: selectedKnowledge?.title || null,
...
export const createAiActionEngine = ({ navigate }) => {
  ...
  const handlers = {
    ...
    get_site_context: async (args = {}) => {
      const scope = typeof args.scope === 'string' ? args.scope.trim() : ''
      const limit = toInt(args.limit, 20)
      const acceptedOnly = args.accepted_only === true
      if (!scope) return { ok: false, reason: 'query_missing' }
      ...
      try {
        ...
        const normalizedScope = scope === 'posts' ? 'publications' : scope
        const qs = new URLSearchParams()
        qs.set('scope', normalizedScope)
        qs.set('limit', String(Math.max(1, Math.min(100, limit))))
        if (normalizedScope === 'submissions') {
          qs.set('accepted_only', acceptedOnly ? 'true' : 'false')
        }

        const resp = await fetch(`${API_URL}/ai/site-context?${qs.toString()}`, {
          method: 'GET',
          headers,
        })
        if (!resp.ok) return { ok: false, reason: 'backend_error' }
        ...
        if (scope === 'staff') {
          const raw = Array.isArray(payload?.staff) ? payload.staff : []
          const rows = raw.slice(0, Math.max(1, Math.min(50, limit)))
          if (!rows.length) {
            return {
              ok: false,
              action: 'get_site_context',
              reason: 'no_result',
              actionSummary: 'Hiện chưa có dữ liệu đội ngũ để hiển thị.',
            }
          }

          const names = rows.map((item, idx) => {
            const name = shorten(item?.name || '', 60) || 'Chưa rõ tên'
            const title = shorten(item?.title || '', 60)
            const expertise = shorten(item?.expertise || '', 60)
            const tail = [title, expertise].filter(Boolean).join(' - ')
            return `${idx + 1}. ${name}${tail ? ` (${tail})` : ''}`
          })
          ...
          return {
            ...
            scope,
            count: raw.length,
            actionSummary: `Đội ngũ hiện có ${raw.length} thầy cô:\n${names.join('\n')}`,
          }
        }

        if (normalizedScope === 'publications') {
          const rows = Array.isArray(payload?.publications) ? payload.publications : []
          if (!rows.length) return { ok: false, action: 'get_site_context', reason: 'no_result', actionSummary: 'Hiện chưa có bài viết phù hợp để hiển thị.' }
          const labels = rows.map((item, idx) => {
            const title = shorten(item?.title || '', 90) || 'Không rõ tiêu đề'
            const path = item?.path || '/'
          ...
          return {
            ...
            count: rows.length,
            actionSummary: `Có ${rows.length} bài viết gần đây:\n${labels.join('\n')}`,
          }
        }

        if (normalizedScope === 'events') {
          const rows = Array.isArray(payload?.events) ? payload.events : []
          if (!rows.length) return { ok: false, action: 'get_site_context', reason: 'no_result', actionSummary: 'Hiện chưa có sự kiện phù hợp để hiển thị.' }
          const labels = rows.map((item, idx) => {
            const title = shorten(item?.title || '', 90) || 'Không rõ tiêu đề'
            return `${idx + 1}. ${title}`
          })
          return {
            ...
            scope: normalizedScope,
            count: rows.length,
            actionSummary: `Có ${rows.length} sự kiện:\n${labels.join('\n')}`,
          }
        }
        ...
        if (normalizedScope === 'submissions') {
          ...
          if (!rows.length) {
            return {
              ...
              action: 'get_site_context',
              reason: 'no_result',
              actionSummary: acceptedOnly
                ? 'Hiện chưa có bài dự thi đã duyệt để hiển thị.'
                : 'Hiện chưa có bài dự thi phù hợp để hiển thị.',
            }
          }
          const labels = rows.map((item, idx) => {
            const title = shorten(item?.title || '', 90) || 'Không rõ tiêu đề'
            const author = shorten(item?.student_name || '', 50)
          ...
          return {
            ...
            actionSummary: acceptedOnly
              ? `Có ${rows.length} bài dự thi đã duyệt:\n${labels.join('\n')}`
              : `Có ${rows.length} bài dự thi:\n${labels.join('\n')}`,
          }
        }
        ...
        return {
          ...
          action: 'get_site_context',
          scope: normalizedScope,
          actionSummary: 'Đã lấy ngữ cảnh website theo yêu cầu.',
        }
      } catch (e) {
        return { ok: false, reason: 'tool_exec_error', message: e?.message || String(e) }
      }
