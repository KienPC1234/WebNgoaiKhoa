...
const shouldInitPush = () => {
  ...
  const hasVapid = Boolean(VAPID_PUBLIC_KEY)
  const shouldInit = Boolean(token) && hasVapid

  if (!shouldInit) {
    log('shouldInitPush() = false', { hasAuthToken: Boolean(token), hasVapidKey: hasVapid })
  }
...
export const initWebPush = async ({ requestPermission = false } = {}) => {
  log('initWebPush() start', { requestPermission })

  if (!shouldInitPush()) {
    warn('Skip — missing token or VAPID key')
    return null
  }
  ...
  if (!VAPID_PUBLIC_KEY) {
    errorLog('Missing VITE_VAPID_PUBLIC_KEY')
    return null
  }
